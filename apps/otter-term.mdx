...
`otter-term` is Otter Shell's built-in terminal emulator. It uses the vendored `ghostty-vt` Zig module for terminal emulation and Otter libraries for Wayland windowing, PTY handling, input, rendering, configuration, and clipboard. It supports tabs, horizontal and vertical splits, OSC shell integration, and IME input. No X11 dependency or installed `libghostty-vt` library is needed.
