...
`otter-term` is a regular `xdg_toplevel` window. It runs on Hyprland, Sway, Niri, and other wlroots-based compositors. Niri scroll damage is tracked explicitly so scrollback redraws stay in sync with the viewport.
