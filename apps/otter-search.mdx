...
`otter-search` indexes personal files and answers fuzzy path queries plus grep-style content search. Indexing and scoring run in pure Zig (`otter_tools_search` in otter-tools-core). It does not use Rust or a vendored fuzzy engine.
...
The daemon watches the filesystem and reindexes on changes. Writes are debounced and throttled so a burst of activity does not thrash the index.
