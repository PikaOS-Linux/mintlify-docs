...
Or install `otter-hud` alone. The launcher works without [otter-benchd](/apps/otter-bench); without the daemon you get a diagnostic banner and the game still runs.
...
- **Fails open.** If the daemon is missing, a layout is bad, or the ring is full, the game keeps running. Instrumentation goes quiet instead of taking the process down.
