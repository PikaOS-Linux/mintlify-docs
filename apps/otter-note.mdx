...
When `notes_path` is empty, otter-note stores notes at `$XDG_DATA_HOME/otter-shell/notes.md`, falling back to `~/.local/share/otter-shell/notes.md` if `XDG_DATA_HOME` is unset. Set `notes_path` to an absolute path to store notes elsewhere, like a synced folder.
