...
`otter-timer` counts down a duration and pings the desktop through `otter-osd` when that is available. It always prints `timer done` when it finishes. You can set a custom notification message, or run a shell command when the timer ends.
