...
`otter-weather` fetches current conditions and a 7-day forecast from Open-Meteo. It feeds the bar weather widget or opens a popup. Set latitude and longitude, then run it as a watch daemon if you want the bar to stay current.

## Usage

Run `otter-weather` with no arguments to fetch and print a forecast for the configured location. Pass coordinates to override the config.
