...
`otter-assist` runs the local model behind Otter Assistant. It listens on a Unix socket tied to your Wayland display and does inference on-device. It can also look up packaged knowledge, search the web, fetch weather, or query desktop search. The chat window is [otter-assistant](/apps/otter-assistant).

<Note>
  `otter-assist`, `otter-assistant`, `otter-assist-data`, and `otter-assist-model` ship in **`otter-shell-extras`**, not the base `otter-shell` metapackage. Install with `pikman install otter-shell-extras`.
</Note>
...
| `otter-assist-data` | Packaged knowledge indexes under `/usr/lib/otter-assist/` |
| `otter-assist-model` | GGUF weights under `/usr/share/otter-shell/models/assist` |
| `otter-assistant` | XDG toplevel chat client |

Packaged builds load the model from `/usr/share/otter-shell/models/assist` (`otter-assist-model`). Indexes land under `/usr/lib/otter-assist/indexes/` (`otter-assist-data`).
