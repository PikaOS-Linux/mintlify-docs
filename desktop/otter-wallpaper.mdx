...
`otter-wallpaper` draws backgrounds on the Wayland background layer using `zwlr_layer_shell_v1`. It decodes each image, draws to an SHM buffer, then frees the decoded data right away to keep memory use low.
...
When the wallpaper changes, `otter-wallpaper` writes the current path for each output to:
