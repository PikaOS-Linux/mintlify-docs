...
description: "Wayland display manager with PAM authentication, fingerprint support, and AccountsService user discovery."
---

`otter-greeter` is Otter Shell's Wayland display manager. It handles login before your user compositor session starts.
...
The greeter UI runs on **Hyprland** configured for the login screen only. Your user session uses whichever compositor you configure separately — the greeter does not dictate your desktop compositor.
...
- Autologin is attempted once per daemon process per seat; failure falls back to interactive login
