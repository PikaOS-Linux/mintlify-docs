...
`otter-osd` shows brief on-screen indicators for volume, brightness, and lock keys. Run it as a daemon; scripts, the bar, and compositor keybindings send commands over a UNIX socket.
...
[otter-timer](/apps/otter-timer) and [otter-rec](/apps/otter-rec) also talk to `otter-osd` for completion notifications.

## Architecture

One binary, two roles:

- **Daemon** (no args) — Connects to Wayland, creates a UNIX datagram socket, renders overlays on demand. Surfaces are created lazily and destroyed after timeout so idle RAM stays near zero.
