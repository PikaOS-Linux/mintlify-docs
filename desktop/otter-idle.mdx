...
description: "Idle management with configurable timeouts, DPMS control, and systemd-logind sleep integration."
---

`otter-idle` detects user inactivity and runs configurable commands — typically locking the session or turning off displays. It integrates with systemd-logind for sleep events and screen saver inhibition.
...
Three layers of idle inhibition:
...
Set `ignore_dbus_inhibit = true` or `ignore_systemd_inhibit = true` to bypass respective layers.

## Logind integration

Responds to logind `Lock`, `Unlock`, and `PrepareForSleep` signals. Default `lock_cmd` ensures [otter-lock](/desktop/otter-lock) is available when a lock is requested.
