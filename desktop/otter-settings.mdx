...
The **Assist** tab edits `otter-assist.conf`. That app is workspace-only today (not in the `otter-shell` metapackage). Fields:

- **Backend**: `cpu` or `vulkan`
- **Vulkan device**: GPU index, or `-1` for the default device
- **Idle seconds**: how long the assist UI waits before going idle (default `120`)
