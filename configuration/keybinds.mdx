...
| otter-launcher | Move through results | `Up` / `Down`, `Tab`, `Page Up/Down` |
| otter-launcher | Clear search | `Ctrl+U` |
| otter-overview | Close / clear search | `Escape` |
| otter-overview | Launch selected app | `Enter` |
| otter-overview | Move through tiles or apps | Arrows / `Tab`, `Page Up/Down` |
| otter-overview | Jump to workspace card | `Ctrl+1` ... `Ctrl+9` (`Ctrl+0` = tenth) |
| otter-switcher | Advance selection | Run `otter-switcher` again |
| otter-switcher | Focus selected | Release Alt, or Enter |
| otter-switcher | Cancel | `Escape` |
| otter-keybindhelp | Close | `Escape` |
| otter-keybindhelp | Search / clear | Type, `Ctrl+U` |
| otter-keybindhelp | Change context / page | `Tab` / `Shift+Tab`, `Page Up` / `Page Down` |
| otter-files | Connect to Server | `Ctrl+K` |
...
Bind these in your compositor config. Change modifiers to match your setup.
...
bind = $mainMod SHIFT, slash, exec, otter-keybindhelp
bind = $mainMod, P, exec, otter-screenshot
bind = $mainMod SHIFT, P, exec, otter-screenshot --annotate
...
bind = $mainMod, W, exec, otter-settings --wallpaper-picker
bind = $mainMod SHIFT, W, exec, otter-wallpaper --random
bind = $mainMod SHIFT, N, exec, otter-notifications dnd-toggle
...
bindsym $mod+Shift+slash exec otter-keybindhelp
bindsym $mod+p exec otter-screenshot
bindsym $mod+Shift+p exec otter-screenshot --annotate
...
bindsym $mod+w exec otter-settings --wallpaper-picker
bindsym $mod+Shift+w exec otter-wallpaper --random
bindsym $mod+Shift+n exec otter-notifications dnd-toggle
bindsym $mod+l exec loginctl lock-session
```

## Recommended autostart

Add these to compositor autostart so the shell is ready at login. See [Session setup](/desktop/session-setup) for compositor-specific syntax.
