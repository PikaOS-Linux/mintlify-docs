...
**`otter-settings`** is the main config UI. One tab per app, a theme browser, and drag-and-drop bar layout. It writes the same `~/.config/otter-shell/*.conf` files you would edit by hand, so the GUI and text stay in sync.
...
| Plugins | Enable or disable installed `.so` plugins |
| Dock | Magnification dock layout and pinned apps |
| Taskbar | Taskbar height, clock, tray, pinned apps |
| Overview | Workspace overview card size, axis, captures |
...
| Files | File manager view, sort, thumbnails, confirms |
| Night Light | Schedule and color temperature |
...
Compositor hotkeys (screenshot, launcher, overview, lock) are **not** in otter-settings. Bind those in your compositor config. See [Keybinds](/configuration/keybinds).

## Manual config files

Each app has a plain-text file in `~/.config/otter-shell/`. Edit by hand when you want to script changes, or keep comments the UI might not show.
...
| Plugins | `~/.config/otter-shell/plugins.conf` |
| otter-overview | `~/.config/otter-shell/otter-overview.conf` |
| otter-dock | `~/.config/otter-shell/otter-dock.conf` |
| otter-taskbar | `~/.config/otter-shell/otter-taskbar.conf` |
...
| otter-nightlight | `~/.config/otter-shell/otter-nightlight.conf` |
...
| otter-files | `~/.config/otter-shell/otter-files.conf` |
...
| otter-pkg | *(app state under `$XDG_STATE_HOME/otter-pkg/`, not otter-conf)* |
| otter-keybindhelp | *(reads compositor configs; no otter-conf file)* |
