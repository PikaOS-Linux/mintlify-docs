...
Changes land in `~/.config/otter-shell/theme.conf`. Running apps that watch the file pick them up. [otter-term](/apps/otter-term) hot-reloads `theme.conf` without a restart.
...
When enabled, `otter-theme-gen` watches `$XDG_RUNTIME_DIR/otter-shell/wallpaper-state` and updates `theme.conf`. You can sync colors to other apps through template outputs: terminals, GTK, Hyprland (`~/.config/hypr/otter-theme.lua`), `bottom`, btop, and more. Toggle each template in the Theme Gen tab, then click **Apply**.
