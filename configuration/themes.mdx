...
All Otter Shell apps share one theme via `otter-theme`. The **Theme** tab in [otter-settings](/desktop/otter-settings) lets you choose a visual profile, density, and color source. It also previews all 12 built-in presets.

```bash
otter-settings
```

Settings stores those choices as ordered imports in `~/.config/otter-shell/theme.conf`. Built-in preset colors go in `preset-colors.conf`. Running apps watch the root file and every imported layer. [otter-term](/apps/otter-term) reloads without a restart, and [otter-bar](/desktop/otter-bar) refreshes widget colors without reloading `otter-bar.conf`.
...
1. Compiled Current fallback
2. Selected profile, `profiles/current.conf` or `profiles/bank.conf`
3. Selected density profile
4. Optional `preset-colors.conf` or `generated-colors.conf`
5. Explicit fields in root `theme.conf`
6. Per-app and per-widget overrides
...
Colors can follow your wallpaper. Run [otter-theme-gen](/desktop/otter-theme-gen) with [otter-wallpaper](/desktop/otter-wallpaper), then use the **Theme Gen** tab in [otter-settings](/desktop/otter-settings). It edits `otter-theme-gen.conf`. The daemon ships in `otter-shell`.
