...
You can build a new Wayland app on the Otter Shell stack. Same libraries, same config format.
...
| `.overlay` (on demand) | Overlay session, exits on dismiss | `otter-launcher`, `otter-logout`, `otter-overview` |
...
A common overlay pattern is `DimOverlay` (fullscreen dim on `.overlay`) plus a second layer surface for the panel. See `otter-launcher` and `otter-logout`. Overlay sessions that exit after one action often use `ArenaAllocator`.
