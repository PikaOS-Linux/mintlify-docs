...
`otter-shell-plugins` Depends on `otter-bluetooth-plugin`. That separate package installs `plugin:bluetooth-applet` under `/usr/lib/otter-shell/plugins/bluetooth-applet/` (`provides_bar_widget`). Advanced device management uses Blueman. Source repo is [bluetooth-applet-plugin](https://git.pika-os.com/otter-shell/bluetooth-applet-plugin); the Debian binary is `otter-bluetooth-plugin`.
...
- [bluetooth-applet-plugin](https://git.pika-os.com/otter-shell/bluetooth-applet-plugin): source for Debian `otter-bluetooth-plugin` (`plugin:bluetooth-applet`)
