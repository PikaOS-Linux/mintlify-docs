...
Enable gates live in `~/.config/otter-shell/plugins.conf` as `<id>_enabled = true|false`. A missing key means enabled. Hosts read that file before `dlopen`. Per-plugin files under `~/.config/otter-shell/plugins/<id>.conf` are reserved for schemas with `provides_settings = true`. That settings UI is not in yet.
...
Bar plugins: use `otter-bar`'s `zig build plugin-abi-smoke -- <so>`.
