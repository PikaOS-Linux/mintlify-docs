...
`otter-utils` is the bottom of the Otter Shell stack. Pure Zig, no external deps. Every library and app links it for bounded containers, path helpers, logging, file watching, and shared XDG paths under `~/.config/otter-shell/`.
...
| `assist` | ChatML prompt helpers for workspace `otter-assist` |
