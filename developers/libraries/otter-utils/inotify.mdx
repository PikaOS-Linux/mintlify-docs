...
`inotify.Watcher` wraps Linux inotify for config reloads and app-list updates. Poll `getFd()` next to your Wayland socket and timer FDs. That is the same pattern `otter-conf.Watcher` uses.
