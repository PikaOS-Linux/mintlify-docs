...
`otter-conf` parses flat `key = value` files into Zig structs. The parser is generated at comptime from your struct shape. Files load through memory-mapped I/O.

Otter Shell apps define schemas in `otter-config-types` and load them with `otter-conf`.
