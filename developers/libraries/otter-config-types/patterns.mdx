...
Shared conventions for config structs and settings tabs. Follow these so `otter-conf` and `otter-settings` stay in sync.
...
Keep workspace-only apps (`otter-hypr`, `otter-cue`) out of packaged install docs unless explicitly marked.
