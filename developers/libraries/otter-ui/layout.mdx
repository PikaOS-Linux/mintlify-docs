...
Layout is all integers. No heap during measure or place. Every node has a `LayoutSpec`. Containers (`row`, `column`, `stack`, `grid`, `scroll_area`) place children inside the rect the parent already got.
