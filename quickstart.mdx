...
<Steps>
  ...
  <Step title="Start background daemons">
    ...
    Optional: `otter-dock &` or `otter-taskbar &` for a bottom app strip.
  ...
  <Step title="Open the workspace overview">
    Bind Super, or another spare key:

    ```bash
    otter-overview
    ```

    Workspace cards, window tiles, search, and an app grid. See [otter-overview](/desktop/otter-overview).
  </Step>

  <Step title="Bind Alt-Tab">
    The same package ships `otter-switcher`:

    ```bash
    otter-switcher
    ```

    Bind Alt-Tab to it. Each press advances. Release Alt to focus. See [otter-switcher](/desktop/otter-switcher).
  </Step>
