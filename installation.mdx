...
description: "Install Otter Shell with pikman. `otter-shell` is the full desktop; `otter-shell-extras` adds speech tools if you want them."
---

Otter Shell ships as native packages on PikaOS. Everything is built and versioned together in the [otter-zenith](https://git.pika-os.com/otter-shell/otter-zenith) repository. Use **pikman** to install.

## System requirements

- **PikaOS** — Otter Shell packages target PikaOS repositories.
- **Wayland compositor** — Otter Shell is Wayland-only. Pick any compositor that supports the protocols your apps need (for example `zwlr_layer_shell_v1` for the status bar, launcher, and most overlays).
- **No Zig runtime needed** — Apps ship as native binaries. You don't need Zig installed.
...
<Steps>
  ...
  <Step title="Install Otter Shell">
    ...
    This pulls every component at the same version. See the package table below for the full list.
...
A terminal window should open on your Wayland session. Then follow the [Quick Start](/quickstart) and [Session setup](/desktop/session-setup) guides to autostart shell daemons.
